...
class Obstacle:
    def __init__(self, start= [0,0], goal = [10,10], X=10, Y=10 ) -> None:
        ...
        self.X = X
        self.Y = Y
        self.obstacle_level=0.15
        self.obstacles = []

    def make_obstacle(self):
        for i in range(10):
            (x, y) = (random.random()*10, random.random()*10)
            polygon = shapely.geometry.Polygon([(x + 0.25, y + 0.25), (x - 0.25, y + 0.25), (x - 0.25, y - 0.25), (x + 0.25, y - 0.25)])
...
class RRT():
    def __init__(self, start, goal, threshold, max_iter=500):
        ...
        ob.plot_obstacles(self.ax)  # yeh kyun ni chalra ab
